...
- `docs/deload-weeks.md` — the design for deload weeks: how a programme week is calculated, where the deload declaration lives, who is allowed to set it, and three pieces of existing plumbing that would swallow the feature silently. **Read before adding any field to `WorkoutPlan` that is meant to change after the plan is created, and before touching `SyncService._pullWorkoutPlans` or `_syncUpdatePlan`** — the plan pull skips every column of a plan it already holds, so a trainer's edit never reaches a device that pulled the plan once; the trainee's plan push is a full-document PUT that will overwrite a field it never edited; and `assignedByTrainer` crosses the wire on both plan and workout DTOs and is discarded by the Flutter client, so the trainee app cannot tell a trainer-assigned plan from its own. A week is anchored on the plan's `startDate` (the console's attendance bars are Monday-anchored and are a *different* week), computed on date-only values because DST makes `.inDays` on instants wrong twice a year, and a past session's deload state is stamped rather than re-derived.
