...
- `docs/play-track-visibility.md` — why the app stopped appearing in Play search, and the rule it leaves behind. **Read before diagnosing anything about the app's availability, discoverability or track configuration** — nothing in this repo can see Play Console state, so findings from here describe only what we ship into it; the Console's track summary reports *staged* rather than published configuration, tester opt-in is per track and not per app, closed testing is never publicly searchable to anyone, and an unsubmitted country list can stall the 12-tester/14-day clock that gates production access. It also records a wrong diagnosis that reached a commit, and why a good fit between symptoms and a mechanism is not evidence.
