...
- `docs/trainer-console-micronutrients.md` — three pre-existing defects in the trainee app's own micronutrient feature (a unit label that was just a string, a rescale that assumed every food's stored blob was per-100g, an edit that silently abandoned a food's micronutrients), and the rules micronutrient tracking on the Trainer Console's Nutrition tab and the trainee diary leaves behind. **Read before touching `ExtendedNutrients`, `NutrientDef`/`nutrient_defs.dart`, `TrainerNutrientPin`, or the micronutrient fields on `ClientNutritionSummaryDto`/`LoggedMealDto`/`LoggedFoodDto`** — every value is stored in grams regardless of source and converted only at the display boundary; a nutrient nobody reported must stay null through every fold, never becoming a reported zero; the day total is folded from the already-deduplicated `loggedMeals`, never from a second traversal of the raw rows, for the same reason `docs/trainer-console-duplicate-rows.md` exists; a pin write always replaces the whole set, never edits one row; and the premium gate is enforced server-side, for both the trainer and the client, with values absent from the payload rather than merely hidden when locked.
- `docs/deload-weeks.md` — the design for deload weeks: how a programme week is calculated, where the deload declaration lives, who is allowed to set it, who is entitled to see or edit it, and pieces of existing plumbing that would swallow the feature silently. **Read before adding any field to `WorkoutPlan` that is meant to change after the plan is created, before touching `SyncService._pullWorkoutPlans` or `_syncUpdatePlan`, and before writing any deload-related copy** — the plan pull skips every column of a plan it already holds, so a trainer's edit never reaches a device that pulled the plan once; the trainee's plan push is a full-document PUT, which is why deload weeks get their own endpoint instead of joining it; `assignedByTrainer` crosses the wire on both plan and workout DTOs and is discarded by the Flutter client, so the trainee app cannot tell a trainer-assigned plan from its own; and an omitted `deloadWeeks` in a payload means "not provided," never "clear it," because the field is deliberately absent when the reader isn't entitled to see it — a reconcile that treats absence as empty deletes a lapsed subscriber's deload weeks and re-subscribing never brings them back. A week is anchored on the plan's `startDate` (the console's attendance bars are Monday-anchored and are a *different* week), computed on date-only values because DST makes `.inDays` on instants wrong twice a year, and a past session's deload state is stamped rather than re-derived. Setting a deload is premium — gated in `PremiumFeatures` and the paywall like the other plan-structure features — but a trainer-set deload always renders for a non-entitled client; a lapsed licence must never delete or hide a programme's own recovery week. The literature (Bell et al. 2024/2025, Bosquet et al. 2007) puts training volume ahead of load as the lever and holds frequency constant, the opposite of the intuitive design, and the evidence that a deload improves outcomes over the short trial horizons studied so far is thin — so no copy anywhere in the feature may claim a deload produces better results than skipping one.
