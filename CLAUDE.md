...
- Onboarding runs **pre-auth**, so the role isn't knowable there — it's skipped entirely on web, and the welcome page has a "Sign in" escape for anyone who already has an account. Moving profile setup behind auth is the outstanding fix; see `docs/onboarding-and-roles.md`.
