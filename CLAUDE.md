...
- `docs/trainer-console-micronutrients.md` — four pre-existing defects in the trainee app's own micronutrient feature (a unit label that was just a string, a rescale that assumed every food's stored blob was per-100g, an edit that silently abandoned a food's micronutrients, and a call site that rebuilt a food row by hand and listed only its macros), and the rules micronutrient tracking on the Trainer Console's Nutrition tab and the trainee diary leaves behind. **Read before touching `ExtendedNutrients`, `NutrientDef`/`nutrient_defs.dart`, `TrainerNutrientPin`, or the micronutrient fields on `ClientNutritionSummaryDto`/`LoggedMealDto`/`LoggedFoodDto`** — every value is stored in grams regardless of source and converted only at the display boundary; a nutrient nobody reported must stay null through every fold, never becoming a reported zero; the day total is folded from the already-deduplicated `loggedMeals`, never from a second traversal of the raw rows, for the same reason `docs/trainer-console-duplicate-rows.md` exists; a pin write always replaces the whole set, never edits one row; the premium gate is enforced server-side, for both the trainer and the client, with values absent from the payload rather than merely hidden when locked; and a `FoodItemData` becomes a `FoodItemModel` only through `FoodItemModel.fromData`, never a constructor call spelled out at the call site, because a nullable field added later defaults rather than failing and every older call site keeps silently asserting the data isn't there.
