...
- `docs/sync-concurrent-runs.md` — why an active workout listed every set twice with the twin inputs linked, and why Session Review showed one logged set eight times, and the rules they leave behind. **Read before touching `_runInitialSync`, anything that calls `syncAll`/`pullAll`, the active workout's save, or any endpoint or pull that writes an exercise's sets or set templates** — sync runs on launch *and* every resume, so two runs overlap unless the static in-flight guard joins them; a delete-then-insert is only a replace inside a transaction (the background isolate has its own connection); the client rewrites an exercise's logged sets as fresh rows on every save, so the server's sets batch must *replace*, and the client must always send the whole log; and a stale server copy is healed by re-queuing the device's log, never by pulling it down.
