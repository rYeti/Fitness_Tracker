...
- `docs/paywall-free-trial.md` — why the Premium screen stopped offering the store's free trial after the hosted RevenueCat paywall was replaced, and why Android showed it as "14 days". **Read before touching `PaywallScreen` or anything that reads `introductoryPrice`** — a Play trial arrives as days, Play hides an offer the user can't get while the App Store shows it to everyone, and the button must advertise exactly the option the purchase buys.
- `docs/workout-exercise-order.md` — why a workout's exercise order changed on its own, and the rule it leaves behind. **Read before adding an `update(...)` to any synced table, or a pull path that overwrites clean rows** — `saveCompleteWorkout` rewrote exercise positions without dirtying the rows, so the reorder never pushed and the sync reconcile (which trusts a clean row to match the server) put the old order back; a write that changes a synced row's pushed columns must promote it `synced` → `pendingUpdate`, and listings order by `orderPosition` *then* `id` so a tied slot is at least stable.
- `docs/trainer-exercise-notes.md` — why a trainee's note on an exercise never reached their trainer although both sides had a column for it, and the rule it leaves behind. **Read before touching `ScheduledWorkoutExercise.Notes`, `linkScheduledExerciseToServer`, or any sync path that links a local row to a server row** — a link records which server row this is, not that their contents agree, so it must leave a row pending when a field it carries differs; and a field that starts syncing for the first time has local data *ahead* of the server, so "server wins over a clean row" would erase it on the first pull.
- `docs/trainer-console-micronutrients.md` — four pre-existing defects in the trainee app's own micronutrient feature (a unit label that was just a string, a rescale that assumed every food's stored blob was per-100g, an edit that silently abandoned a food's micronutrients, and a call site that rebuilt a food row by hand and listed only its macros), and the rules micronutrient tracking on the Trainer Console's Nutrition tab and the trainee diary leaves behind. **Read before touching `ExtendedNutrients`, `NutrientDef`/`nutrient_defs.dart`, `TrainerNutrientPin`, or the micronutrient fields on `ClientNutritionSummaryDto`/`LoggedMealDto`/`LoggedFoodDto`** — every value is stored in grams regardless of source and converted only at the display boundary; a nutrient nobody reported must stay null through every fold, never becoming a reported zero; the day total is folded from the already-deduplicated `loggedMeals`, never from a second traversal of the raw rows, for the same reason `docs/trainer-console-duplicate-rows.md` exists; a pin write always replaces the whole set, never edits one row; the premium gate is enforced server-side, for both the trainer and the client, with values absent from the payload rather than merely hidden when locked; and a `FoodItemData` becomes a `FoodItemModel` through `FoodItemModel.fromData`, never a constructor call spelled out at the call site, because a nullable field added later defaults rather than failing and every older call site keeps silently asserting the data isn't there — the two sites that must build one by hand because they *scale* it (the Add-to-Template button and quick-add) still owe it every field the row had. Meal templates carry micronutrients end to end (item model → SharedPreferences blob → both apply paths → the `MealTemplateItems.ExtendedNutrientsJson` column), and the server stores that blob opaquely and must never parse it.
