...
/// Best-effort sweep of the temp directory documents are opened from
/// (`attachment_opener_io.dart`). Not this store's own directory — that one
/// is permanent, app-support storage; this is OS temp, but nothing else in
/// this app clears it, so sign-out is where it's swept. A no-op on web,
/// same as every other native-only helper this store calls.
Future<void> clearOpenedDocumentsTempDir() async {
  if (kIsWeb) return;
  final dir = await getTemporaryDirectory();
  await deleteAttachmentDirectory('${dir.path}/$chatDocsOpenDirName');
}
...
class AttachmentStore {
  ...
  // [id] is `ChatAttachmentRef.id`, client-generated by whoever *sent* the
  // message — not this device. It was interpolated directly into this path
  // (permanent, app-private storage) before `safeAttachmentIdSegment`
  // existed; see that function's own doc comment.
  String _filePath(String base, String id) =>
      '$base/${safeAttachmentIdSegment(id)}.bin';
  ...
  Future<void> clearAll() async {
    ...
    // A different, temp-not-permanent directory — see this function's own
    // doc comment.
    await clearOpenedDocumentsTempDir();
  }
}
