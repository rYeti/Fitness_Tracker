...
class _Pill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // The wash and the icon are StatusBadge's warn tone, taken from it so
    // the two can't drift apart. Text and the Retry label are onSurface
    // rather than amber: amber text on its own tint over the light page
    // measures 4.35:1, short of AA for 12px. The icon keeps the tone (at
    // least 4.35 light and 6.02 dark, against 3:1). contrast_test.dart names
    // each pair.
    return Semantics(
      container: true,
      liveRegion: true,
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: StatusBadge.tintOf(StatusTone.warn, brightness),
          ...
          children: [
            ExcludeSemantics(
              child: Icon(
                ...
                size: 16,
                color: StatusBadge.foregroundOf(StatusTone.warn, brightness),
              ),
            ),
