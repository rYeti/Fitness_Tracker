...
- Fixed deleted workouts still appearing in the Trainer Console's Session Review, listed as missed sessions the client never did. Deleting a workout failed server-side for any workout that had ever been scheduled — the scheduled-session tables hold a restricted foreign key to it — so the workout vanished from the client's list but survived on the server. Deletion now works: sessions that logged nothing are removed with it, and where sets were logged the workout is retired instead of deleted so that history keeps its name. Existing stuck workouts clear themselves on the client's next sync.
- Fixed the Trainer Console showing every logged set two or more times. Saving an exercise rebuilds the client's set rows from scratch, which drops their server ids, so the whole log is pushed as new on each save — and the API appended it instead of replacing it. The trainer saw one copy per save while the client showed the right count. The endpoint now replaces, which also clears the duplicates already stored on the next sync.
- Fixed RPE being blank for every set in the Trainer Console. The value was accepted on the request and stored on the model but mapped in neither direction, so nothing was ever saved or returned. Average RPE per session now works too. Previously logged sets have no RPE to recover.
- Fixed exercises removed from a workout never being deleted server-side, which left them in the trainer's Session Review as exercises the client had skipped. The client marked them for deletion but never marked the workout itself as changed, so the retry never ran. Requires this app update to take effect.
