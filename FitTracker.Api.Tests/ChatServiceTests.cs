...
{
    private static IChatService NewService(ChatTestContext ctx) =>
    ...
    public async Task SendMessage_rejects_a_revoked_relationship()
    {
        ...
        ctx.AddRelationship(ctx.TrainerId, formerClient.Id, Models.TrainerClientStatus.Revoked);
        var service = NewService(ctx);
    ...
    public async Task Conversations_excludes_relationships_that_are_not_active()
    {
        ...
        ctx.AddRelationship(ctx.TrainerId, pending.Id, Models.TrainerClientStatus.Pending);
