...
- A write that changes nothing on the server stamps nothing (§27), so a device
  ...
  rows are clean.
- The first answer, sent without a cursor, carries every tombstone the account
  has (§31). An upgrading install applies them like any others; a fresh one
  finds nothing to delete.
- A 410 answers a create naming a row that was deleted elsewhere:
  `{ "error": "id_deleted", "id": "…", "ids": ["…", …] }`. A foods batch that
  carries deleted ids has applied nothing, and `ids` names every one of them.
  Drop those entries and send the rest. `id` is the first of `ids`.
